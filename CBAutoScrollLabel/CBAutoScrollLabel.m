...
- (void)scrollLabelIfNeeded {
    ...
    [UIView animateWithDuration:duration delay:self.pauseInterval options:self.animationOptions | UIViewAnimationOptionAllowUserInteraction animations:^{
        ...
        if (strongSelf) {
            // adjust offset
            strongSelf.scrollView.contentOffset = (doScrollLeft ? CGPointMake(labelWidth + strongSelf.labelSpacing, 0) : CGPointZero);
        }
    } completion:^(BOOL finished) {
        typeof(weakSelf) __strong strongSelf = weakSelf;
        
        if (strongSelf) {
            strongSelf->_scrolling = NO;
            
            // remove the left shadow
            [strongSelf applyGradientMaskForFadeLength:strongSelf.fadeLength enableFade:NO];
            
            // setup pause delay/loop
            if (finished) {
                [strongSelf performSelector:@selector(scrollLabelIfNeeded) withObject:nil];
            }
        }
    }];
}
